...
# """Convert a docbank-pipeline results.json into a searchable PDF."""
# from __future__ import annotations
# import json
# from pathlib import Path
...
# def results_to_searchable_pdf(
#     results_json: str | Path,
#     output_pdf: str | Path,
#     *,
#     invisible_text: bool = True,
# ) -> Path:
#     """One page per source image. The page image is drawn as background;
#     each recognised detection is overlaid as text at its bbox.

#     invisible_text=True  -> PDF text mode 3 (looks identical to original,
#                             but text is selectable / Ctrl-F searchable).
#     invisible_text=False -> overlay text is visible on top of the image
#                             (useful for debugging OCR alignment).

#     Formulas: the LaTeX source is overlaid as searchable text; the visual
#     formula is whatever was already in the source page image.
#     """
...
#         # Use the original image's pixel dimensions as the PDF page size,
#         # so bboxes map 1:1 without scaling.
#         c.setPageSize((page_w, page_h))
#         c.drawImage(ImageReader(img_path), 0, 0, page_w, page_h)
...
#             t.setFont("Helvetica", font_size)
#             if invisible_text:
#                 t.setTextRenderMode(3)  # invisible but indexable
#             # Start at the top-left of the box, then textLine() walks down.
#             t.setTextOrigin(x1, pdf_y_top - font_size)
#             for line in lines:
#                 # Helvetica is latin-1 only; strip anything it can't encode
#                 # rather than crash. For full Unicode, register a TTF below.
#                 t.textLine(line.encode("latin-1", "replace").decode("latin-1"))
#             c.drawText(t)
