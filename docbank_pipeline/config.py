...
The defaults are safe for a local run; override them by:
  ...
  2. Setting environment variables (DATA_ROOT, MAX_PAGES, TELEGRAM_BOT_TOKEN, ...), OR
...
class PipelineConfig:
    ...
    # ----------------------------------------------------- Telegram bot token
    # Token generated by BotFather. Kept in PipelineConfig so Stage 7 uses the
    # same env-var/config-object pattern as the rest of the pipeline.
    ...
    def describe(self) -> str:
        return (
            ...
            f"LAYOUT_WEIGHTS  = {self.layout_weights or '<unset>'}\n"
            f"HF_TOKEN        = {'<set>' if self.hf_token else '<unset>'}\n"
            f"TELEGRAM_BOT_TOKEN = {'<set>' if self.telegram_bot_token else '<unset>'}\n"
        )
